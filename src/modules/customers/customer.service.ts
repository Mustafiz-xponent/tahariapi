...
export async function getAllCustomers(
  ...
  try {
    ...
    const [sentMessages, receivedMessages, unreadCounts, totalUnread] =
      await Promise.all([
    ...
    const sentMessagesMap = new Map(
      sentMessages.map((m) => [m.senderId.toString(), m]),
    );
    const receivedMessagesMap = new Map(
      receivedMessages.map((m) => [m.receiverId?.toString() || "", m]),
    );
    const unreadCountsMap = new Map(
      unreadCounts.map((u) => [u.senderId.toString(), u._count.messageId]),
    );
