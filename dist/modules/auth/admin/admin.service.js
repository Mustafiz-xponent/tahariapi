...
function generateOtp(length = OTP_LENGTH) {
    ...
    const randomNumber = crypto_1.default.randomBytes(4).readUInt32BE(0) % max;
 ...
 */
async function processAndSendOtp(phone) {
    // ── 1. Enforce resend cooldown ──────────────────────────────────────────
    const cooldownThreshold = new Date(Date.now() - OTP_RESEND_COOLDOWN_SECONDS * 1000);
    const recentOtp = await prismaClient_1.default.otp.findFirst({
        where: {
            phone,
    ...
    const otpHash = await bcrypt_1.default.hash(otp, OTP_HASH_SALT_ROUNDS);
    ...
    logger_1.default.info(`[OTP] Generated for ${phone}. Expires at ${expiresAt.toISOString()}`);
    // ── 5. Deliver OTP via SMS ─────────────────────────────────────────────
    try {
        const message = buildOtpMessage(otp);
        await sendSms(phone, message);
        logger_1.default.info(`[OTP] SMS delivered to ${phone}.`);
    }
    catch (smsError) {
        logger_1.default.error(`[OTP] SMS delivery failed for ${phone}: ${smsError instanceof Error ? smsError.message : "Unknown error"}`);
        if (process.env.NODE_ENV !== "production") {
            logger_1.default.warn(`[OTP] DEV FALLBACK — OTP for ${phone}: ${otp}`);
        }
    }
    return {
        expiresAt,
...
async function validateOtp(phone, otp) {
    ...
    if (!otpRecord) {
        throw new appError_1.AppError("OTP not found or has expired. Please request a new OTP.", http_status_1.default.UNAUTHORIZED);
    }
    // ── 3. Compare submitted OTP against bcrypt hash ───────────────────────
    const isMatch = await bcrypt_1.default.compare(otp, otpRecord.otpHash);
    // ── 4. On mismatch — throw descriptive error ───────────────────────────
    if (!isMatch) {
        logger_1.default.warn(`[OTP] Invalid OTP attempt for ${phone}.`);
        throw new appError_1.AppError("Invalid OTP. Please check the code and try again.", http_status_1.default.UNAUTHORIZED);
    }
    // ── 5. Delete record — enforce single-use ─────────────────────────────
    await prismaClient_1.default.otp.delete({
        where: { otpId: otpRecord.otpId },
    });
    logger_1.default.info(`[OTP] Verified successfully for ${phone}.`);
    return true;
}
// ---------------------------------------------------------------------------
// Admin Management
...
async function loginAdmin(data) {
    try {
        ...
        if (!isValid)
            throw new Error("Invalid credentials.");
        const { passwordHash: _, ...userData } = user;
        logger_1.default.info(`[Admin] Login successful: ${user.phone}`);
...
const adminForgotPassword = async (phone) => {
    ...
    logger_1.default.info(`[Admin] Forgot password OTP sent to ${phone}.`);
...
const adminResetPassword = async (bodyData) => {
    ...
    if (!validRoles.includes(user.role)) {
        throw new appError_1.AppError("You are not permitted to perform this action.", http_status_1.default.FORBIDDEN);
    }
    // ── Ensure new password differs from current ───────────────────────────
    const isSamePassword = await bcrypt_1.default.compare(password, user.passwordHash);
    if (isSamePassword) {
    ...
    logger_1.default.info(`[Admin] Password reset successfully for ${phone}.`);
};
