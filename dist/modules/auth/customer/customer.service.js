...
async function processAndSendOtp(phone) {
    ...
    if (process.env.NODE_ENV === "production") {
        ...
        logger_1.default.info(`[OTP] DEV MODE — OTP for ${phone}: ${otp}`);
    }
    return {
        expiresAt,
...
async function registerCustomer(data) {
    try {
        ...
        const passwordHash = data.password
            ? await bcrypt_1.default.hash(data.password, PASSWORD_SALT_ROUNDS)
            : null;
        ...
        logger_1.default.info(`[Customer] Registered new customer: ${data.phone}`);
...
async function loginCustomer(data) {
    ...
    if (!user.passwordHash) {
        throw new Error("Password login is not enabled for this account. Please use OTP login.");
    }
    // ── 3. Compare password ────────────────────────────────────────────────
    ...
    logger_1.default.info(`[Customer] Password login successful: ${user.phone}`);
    // ── 4. Issue JWT ───────────────────────────────────────────────────────
...
async function otpLoginCustomer(data) {
    ...
    logger_1.default.info(`[Customer] OTP login initiated for ${data.phone}`);
    // ── 2. Send OTP ───────────────────────────────────────────────────────
    const { expiresAt, otp } = await processAndSendOtp(data.phone);
    return { expiresAt, otp };
}
...
async function verifyCustomerOtp(data) {
    ...
    if (user.customer && !user.customer.wallet) {
        ...
        logger_1.default.info(`[Customer] Wallet created for customer: ${data.phone}`);
    }
    ...
    logger_1.default.info(`[Customer] Account activated: ${data.phone}`);
    // ── 5. Issue JWT ──────────────────────────────────────────────────────
...
async function updateCustomerProfile(userId, data) {
    ...
    if (data.password) {
        updateData.passwordHash = await bcrypt_1.default.hash(data.password, PASSWORD_SALT_ROUNDS);
    }
    ...
    logger_1.default.info(`[Customer] Updating profile for userId: ${userId}`);
